...
class GeneratorJIT(nn.Module):
    def __init__(self, mel_channel):
        super(GeneratorJIT, self).__init__()
    ...
    def forward(self, mel):
        ...
        audio = self.generator.forward(mel)
        audio = audio.squeeze() # collapse all dimension except time axis
        audio = audio[:-(hop_length*10)]
        audio = audio.clamp(min=-1, max=1)
        #audio = 32768.0 * audio
        #audio = audio.clamp(min=-32768.0, max=32768.0-1)
        #audio = audio.short()
        audio.dequantize()
        return audio
